...
struct rec_energy_nue {
    ...
    static std::tuple<
        ...
        herr_t err;
        int ndims;
        std::vector<rec_energy_nue> content;
        std::vector<unsigned> col_run;
        std::vector<unsigned> col_subrun;
        std::vector<unsigned> col_evt;
        ...
        for(uint64_t i = 0; i < dims[0]; i++) {
            ...
            content[i].rawgevhad = col_rawgevhad[i];
            content[i].rawgevshw = col_rawgevshw[i];
            content[i].rawgevslice = col_rawgevslice[i];
